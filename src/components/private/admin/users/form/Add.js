...
const initialValues = {
  first_name: '', last_name: '', ci_type: 'V', ci_number: '', email: '', password: '', confirm_password: '', role_id: '' 
}
...
const formSchema = Yup.object().shape({
  ...
  ci_number: Yup.string().required('Debes colocar el número de cédula').matches(/^[0-9]{5,9}$/, 'Número de cédula inválido. Por favor revisa los datos'),
  email: Yup.string().required('Debes colocar el correco electrónico'),
  password: Yup.string().required('Debes colocar una contraseña').matches(/^(?=.*\d)(?=.*[a-zA-Z])[A-Za-z\d!@#$%^&*+=]{6,15}$/, 'Contraseña inválida. Solo se permiten estos caraceres (@#$%^*)'),
  confirm_password: Yup.string().required('Debes confirmar la contraseña').oneOf([Yup.ref('password'), null], 'Las contraseñas no coinciden'),
  role_id: Yup.number().required('Debes seleccionar un rol para este usuario')
})

const AddUser = withRouter(({ registerUser, history }) => {

  return (
    <>
    <section className="content-header form-header">
      <h3 className="font-weight-bold">Agregar nuevo usuario</h3>
    </section>
            ...
            <Form>
              <div className="box-body">
                ...
                <div className="form-group">
                  ...
                  <span className="form-msg">Minimo 6 caracteres y máximo 15.</span>
                </div>
...
export default connect(null, { registerUser })(AddUser);
