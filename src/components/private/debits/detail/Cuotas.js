...
import { getDebitFees } from "../../../../actions/debits";
        ...
        <TableBody>
          {debit_fees.map(fee => (
            <TableRow key={fee.id} hover role='checkbox' tabIndex={-1}>
              ...
              <TableCell>
                {fee.completedDate 
                  ? <Moment format='DD/MM/YYYY'>{fee.completedDate}</Moment>
                  : 'Sin cobrar'
                }
              </TableCell>
          ...
          <TableRow className="total-row">
            <TableCell colSpan={4} align="right">Monto Total</TableCell>
            <TableCell align="left">{amountFormat(total_amount)}</TableCell>
          </TableRow>
          <TableRow className="total-row">
            <TableCell colSpan={4} align="right">Monto restante</TableCell>
