...
import { getTransferDetail, updateTransferP, updateTransferS } from '../../../../actions/activity';
  ...
  return (
    <>
      <section className="content-header">
        <h1>Transferencia <small>#{detail.id}</small> </h1>
      </section>
      <section className="invoice">
        ...
        <div className="row invoice-info">
          <div className="col-sm-4 invoice-col">
            Realizada por:
            <address>
              <strong>{detail.sender_fname + ' ' + detail.sender_lname}</strong><br/>
              {detail.sender_cedula}<br/>
              {detail.sender_email}<br/><br/>
              <strong>Cuenta a debitar:</strong><br/>
              {detail.bank_name}<br/>
              {detail.acc_number}<br/>
              {detail.acc_type}<br/>
            </address>
          </div>
          <div className="col-sm-4 invoice-col">
            Recibe:
            <address>
              <strong>{detail.receiver_fname + ' ' + detail.receiver_lname}</strong><br/>
              {detail.receiver_cedula}<br/>
              {detail.receiver_email}
            </address>
          </div>
          <div className="col-sm-4 invoice-col">
            <b>Transferencia #{detail.id}</b><br/>
            <b>Monto:</b> {Number(detail.amount).toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Bs.<br/>
            <b>Descripción:</b> {detail.description}<br/><br/>
            <span className={`status ${detail.status}`}>{detail.status}</span><br/><br/>
        ...
        <div className="row no-print">
          <div className="col-xs-12">
            ...
            <button type="button" className="btn btn-success pull-right" style={{ marginRight: "5px"}}
              onClick={() => {
